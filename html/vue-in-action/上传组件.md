iView 的上传组件 [Upload](https://www.iviewui.com/components/upload-en) 挺好用的，项目中很多地方都会使用到文件上传功能，如果每次都原生的写一遍相似的代码，一是重复劳动，二是容易出错、修改不易，再者提示不够友好，所以有必要根据业务需求封装一个我们自己的上传组件，就像下图这样:

<img src="img/fileupload.png" width=535>

有以下几个特点:

* 使用更简单: `<FileUpload @on-success="fileUploaded"/>`
* 友好的错误提示
* 统一的文件上传风格
* 可限制不同文件的大小
* 可对上传的图片进行预览
